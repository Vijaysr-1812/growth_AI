...
export default function LoginPage() {
  ...
  const handleCredentialsSubmit = async (e: React.FormEvent) => {
    ...
    if (result?.error) {
      setError(isSignUp ? "Account created, but failed to log in." : "Invalid email or password. Or account not verified.");
      setIsLoading(false);
    } else {
      ...
      <div className="relative z-10 max-w-md w-full mx-auto p-8">
        ...
        >
          ...
          <button
            ...
            className="w-full bg-[#111] border border-white/10 hover:bg-white/5 text-white font-medium py-2.5 rounded-lg text-sm transition-colors flex items-center justify-center mb-6 shadow-sm"
          >
